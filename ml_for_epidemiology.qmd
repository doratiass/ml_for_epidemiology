...
::::: {.column width="50%"}
::: {style="background-color: #f8f9fa; padding: 1.5em; border-radius: 10px; margin: 1em 0;"}
**📈 Scales to Millions of Patients** *Population-level insights.*
:::

::: {style="background-color: #e8f5e8; padding: 1.5em; border-radius: 10px; margin: 1em 0;"}
**🎯 Provides Actionable Predictions:** *Not just correlations—interventions.*
:::
:::::
:::::::::
...
demo_subset <- ml_df %>%
  drop_na() %>%
  #slice_sample(n = 2500) %>%
  mutate(outcome_num = as.numeric(outcome == "centenarian"))

p1 <- ggplot(demo_subset, aes(x = med_sbp_mean, y = outcome_num)) +
...
tree_preds <- demo_subset %>%
  mutate(
    pred = case_when(
      med_sbp_mean < 130 ~ 0.12,
      med_sbp_mean < 150 ~ 0.08,
      TRUE ~ 0.05
    )
  )

p2 <- ggplot(demo_subset, aes(x = med_sbp_mean, y = outcome_num)) +
  geom_jitter(height = 0.05, alpha = 0.6) +
...
p3 <- ggplot(demo_subset, aes(x = med_sbp_mean, y = outcome_num)) +
  geom_jitter(height = 0.05, alpha = 0.6) +
  geom_smooth(
    method = "loess",
    se = FALSE,
