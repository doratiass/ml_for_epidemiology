...
The best penalty (λ) is typically chosen via cross-validation, selecting the value produce the optimal model performance.
:::
...
-   **Bayesian Optimization**: Builds a probabilistic model (e.g., Gaussian Process) of performance across the space and **updates** it iteratively to focus on promising regions.
    -   Good for: Each evaluation is **computationally expensive** (e.g., large datasets)
    -   Smart, data-driven exploration balancing exploration and exploitation.

::: {.callout-tip title="Key Principle"}
The **choice of search space** should align with your **optimization strategy**.\
Different methods explore parameter combinations in distinct ways.
:::
...
**Neural networks form the foundation of Large Language Models (LLMs)**:

-   **GPT, ChatGPT, Claude**: Built on transformer neural network architectures
-   **Deep learning**: Neural networks with many hidden layers
-   **Attention mechanisms**: Advanced neural network components for processing sequences
-   **Same principles**: Pattern recognition through weighted connections and learning

### In Epidemiology

**Can be used for traditional prediction tasks**: - Disease outcome prediction - Risk stratification - Medical image analysis - Time series forecasting (disease outbreaks)
:::
:::::
...
**For most epidemiological prediction tasks, start with simpler methods (logistic regression, XGBoost) before considering neural networks**
:::
:::::
...
While neural networks power modern AI breakthroughs like LLMs, they're not always the best choice for epidemiological prediction. **Match your method to your data type and research question.**
:::
